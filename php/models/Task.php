...
class Task extends \yii\db\ActiveRecord
{
    ...
    public function beforeSave($insert)
    {
        ...
        $worker1Count = Task::find()->where(['worker' => $worker1Name])->count();
        $worker2Count = Task::find()->where(['worker' => $worker2Name])->count();
        if ($worker1Count > $worker2Count) {
            $this->worker = $worker2Name;
        } else {
            $this->worker = $worker1Name;
        }
