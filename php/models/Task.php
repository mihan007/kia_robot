...
class Task extends \yii\db\ActiveRecord
{
    ...
    {
        $totalWorkers = Yii::$app->params['totalWorkers'];
        $workersCount = [];
        for ($i=1; $i<= $totalWorkers; $i++) {
            $workerName = 'worker-'.$i;
            $workersCount[$i] = Task::find()
                ->where(['worker' => $workerName])
                ->andWhere(['deleted_at' => 0])
                ->count();
        }
        $minKeys = array_keys($workersCount, min($workersCount));
        $minKey = reset($minKeys);
        $minWorkerName = 'worker-'.$minKey;
        $this->worker = $minWorkerName;
